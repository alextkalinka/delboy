...
#' @param lfc_shrink Logical - shrink logFC values, or not (default).
#'
...
run_deseq2 <- function(data, group_1, group_2, gene_column, lfc_shrink = T){
  tryCatch({
    ...
    if(lfc_shrink){
      suppressMessages(res <- DESeq2::lfcShrink(dds, coef = "tr_group_2_vs_group_1", 
                                                res = DESeq2::results(dds), type = "normal"))
    }else{
